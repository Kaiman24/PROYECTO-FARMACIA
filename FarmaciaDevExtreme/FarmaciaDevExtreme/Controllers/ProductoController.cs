...
namespace FarmaciaDevExtreme.Controllers
{
    public class ProductoController : ApiController
    {
        ...
        loadOptions)
        {
            var apiUrl = "https://localhost:44329/api/Productoes/";
            var respuestaJson = await GetAsync(apiUrl);
            //System.Diagnostics.Debug.WriteLine(respuestaJson);

            List<Producto> listaProducto = JsonConvert.DeserializeObject<List<Producto>>(respuestaJson);
            return
        ...
        form)
        {
            ...
            var apiUrlGetPeli = "https://localhost:44329/api/Productoes/" + key;
            var respuestaPelic = await GetAsync(apiUrlGetPeli = "https://localhost:44329/api/Productoes/" + key);
            Producto producto = JsonConvert.DeserializeObject<Producto>(respuestaPelic);
            JsonConvert.PopulateObject(values, producto);
            string jsonString = JsonConvert.SerializeObject(producto);
            var httpContent = new StringContent(jsonString, System.Text.Encoding.UTF8, "application/json");
            var url = "https://localhost:44329/api/Productoes/" + key;
        ...
        public async Task<HttpResponseMessage> Delete(FormDataCollection form)
        {
            ...
            var apiUrlDelPeli = "https://localhost:44329/api/Productoes/"
                + key;
